...
static BOOL BASS_INIT_ONCE()
{
	...
	BASS::stream_sounds.fatality_hitsound = BASS_StreamCreateFile(TRUE, fatality_hitsound, 0, sizeof( fatality_hitsound ), 0);
	BASS::stream_sounds.bubble_hitsound = BASS_StreamCreateFile(TRUE, bubble_hitsound, 0, sizeof( bubble_hitsound ), 0);
		...
		if (meta) { // got Icecast/OGG tags
			...
			if (title) {
				if (artist) {
					...
					//sprintf(text, sizeof(text), "%s - %s", artist, title);
