...
void SimulationContext::InitSimulationContext( LagRecord_t *record ) {
	static auto sv_maxspeed = g_pCVar->FindVar( XorStr( "sv_maxspeed" ) );
	static auto sv_stepsize = g_pCVar->FindVar( XorStr( "sv_stepsize" ) );

	this->walking = false;
	this->m_vecOrigin = record->m_vecPredOrigin;
	this->m_vecVelocity = record->m_vecPredVelocity;
	this->simulationTicks = record->m_flSimulationTime;
	this->trace = CGameTrace{};
	this->gravity = g_Vars.sv_gravity->GetFloat( ) * g_pGlobalVars->interval_per_tick;
	this->sv_jump_impulse = ( g_Vars.sv_jump_impulse )->GetFloat( );
	this->stepsize = sv_stepsize->GetFloat( );
	this->pMins = record->m_vecMins;
	this->pMaxs = record->m_vecMaxs;
	this->player = record->m_pEntity;
	this->filter = &world_filter;
	this->flags = record->m_fPredFlags;

	float maxSpeed = record->m_pEntity->GetMaxSpeed( );
...
#define MAX_CLIP_PLANES 5 
	...
	allFraction = 0;
	time_left = g_pGlobalVars->interval_per_tick;   // Total time for this movement operation.
...
#define CS_PLAYER_SPEED_DUCK_MODIFIER 0.34f
	...
	static auto sv_enablebunnyhopping = g_pCVar->FindVar( XorStr( "sv_enablebunnyhopping" ) );
	static auto sv_walkable_normal = g_pCVar->FindVar( XorStr( "sv_enablebunnyhopping" ) );
	...
	if( this->flags & FL_ONGROUND ) {
		...
		// Friction
		{
			static auto sv_stopspeed = g_pCVar->FindVar( XorStr( "sv_stopspeed" ) );
			static auto sv_friction = g_pCVar->FindVar( XorStr( "sv_friction" ) );
			...
			if( speed <= 0.1f ) {
				...
				auto drop = ( control * friction ) * g_pGlobalVars->interval_per_tick;
		...
		// WalkMove
		{
			...
			this->m_vecVelocity.z = 0.0f;
			{
				...
				if( addspeed > 0.0f ) {
					...
					if( sv_accelerate_use_weapon_speed && sv_accelerate_use_weapon_speed->GetBool( ) && weapon ) {
						auto weaponData = weapon->GetCSWeaponData( );
					...
					float v27 = ( ( g_pGlobalVars->interval_per_tick * accel ) * wihspeedClamped );
					...
					float v31 = fminf( ( ( g_pGlobalVars->interval_per_tick * v30 ) * wihspeedClamped ), addspeed );
					addspeed = v31;
			...
			Vector dest;
			dest[ 0 ] = this->m_vecOrigin[ 0 ] + this->m_vecVelocity[ 0 ] * g_pGlobalVars->interval_per_tick;
			dest[ 1 ] = this->m_vecOrigin[ 1 ] + this->m_vecVelocity[ 1 ] * g_pGlobalVars->interval_per_tick;
		...
		if( addspeed > 0.0 ) {
			...
			auto v170 = std::fminf( addspeed, v169 * g_pGlobalVars->interval_per_tick );
