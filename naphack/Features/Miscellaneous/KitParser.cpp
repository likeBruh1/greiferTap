...
void KitParser::Initialize( ) {
	// We need these for overriding viewmodels and icons
	g_KitParser.vecWeaponInfo = {
	};

	vecWeaponInfo[ WEAPON_KNIFE ] = { ( "models/weapons/v_knife_default_ct.mdl" ), ( "models/weapons/w_knife_default_ct.mdl" ), ( "knife" ) };
	vecWeaponInfo[ WEAPON_KNIFE_T ] = { ( "models/weapons/v_knife_default_t.mdl" ), ( "models/weapons/w_knife_default_t.mdl" ), ( "knife_t" ) };
	...
	// Dump paint kits
	{
		...
		for( auto i = 0; i <= map_head->last_element; ++i ) {
			...
			if( ( paintKit->id >= 415 && paintKit->id <= 421 ) ||
				( paintKit->id >= 617 && paintKit->id <= 619 ) ||
				( paintKit->id >= 568 && paintKit->id <= 572 ) ) {
				auto Format = [ & ] ( const std::string &str ) -> std::string {
					const int nFoundPhase = str.find( XorStr( "phase" ) );
					const bool bFoundRuby = str.find( XorStr( "ruby" ) ) != std::string::npos;
					const bool bFoundSapphire = str.find( XorStr( "sapphire" ) ) != std::string::npos;
					const bool bFoundEmerald = str.find( XorStr( "emerald" ) ) != std::string::npos;
					const bool bFoundBlackPearl = str.find( XorStr( "blackpearl" ) ) != std::string::npos;

					if( nFoundPhase != -1 ) {
						const auto substr = str.substr( nFoundPhase );
						for( char const &c : substr ) {
							if( std::isdigit( c ) ) {
								std::string phase = XorStr( " (Phase " );
								phase += c;

								return phase.append( XorStr( ")" ) );
							}
						}
					}
					else if( bFoundRuby ) {
					...
					else if( bFoundSapphire ) {
						return XorStr( " (Sapphire)" );
					}
					else if( bFoundEmerald ) {
						return XorStr( " (Emerald)" );
					}
					else if( bFoundBlackPearl ) {
						return XorStr( " (Blackpearl)" );
					}

					return XorStr( "" );
				};

				std::string_view item_name = paintKit->name.buffer;
				const auto appender = Format( item_name.data( ) );
				name_str.append( appender );
			}

			paint_kit kit;
			kit.id = paintKit->id;
			kit.name = name_str;
			kit.item_name = paintKit->name.buffer;

			for( int i = 0; i <= 3; ++i )
				kit.defColor[ i ] = paintKit->rgbaColor[ i ];

			all_skins.push_back( kit );

			if( paintKit->id < 10000 ) {
				vecPaintKits.push_back( kit );
			}
			else {
				vecGlovePaintKits.push_back( kit );
			}
		}
		...
		if( !vecPaintKits.empty( ) ) {
			...
			for( auto i = 0; i < vecPaintKits.size( ); ++i ) {
				std::transform( vecPaintKits[ i ].name.begin( ), vecPaintKits[ i ].name.end( ), vecPaintKits[ i ].name.begin( ), ::tolower );
				g_Vars.globals.m_vecPaintKits.emplace_back( vecPaintKits[ i ].name );
			}
		}

		// copy array contents
		if( !vecGlovePaintKits.empty( ) ) {
			std::sort( vecGlovePaintKits.begin( ), vecGlovePaintKits.end( ) );
			for( auto i = 0; i < vecGlovePaintKits.size( ); ++i ) {
				std::transform( vecGlovePaintKits[ i ].name.begin( ), vecGlovePaintKits[ i ].name.end( ), vecGlovePaintKits[ i ].name.begin( ), ::tolower );
				g_Vars.globals.m_vecGloveKits.emplace_back( vecGlovePaintKits[ i ].name );
			}
		}
	}
