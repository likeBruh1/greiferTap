...
void BeamEffects::Draw( ) {
	...
	for( size_t i{ 0 }; i < m_vecBeamEffects.size( ); i++ ) {
		...
		if( currentImpact.m_bRing ) {
			...
			auto beam = g_pViewRenderBeams->CreateBeamRingPoint( beamInfo );
			if( beam ) {
		...
		if( bFinalImpact || currentImpact.m_nIndex != pLocal->EntIndex( ) ) {
			...
			beamInfo.m_flLife = g_Vars.esp.bullet_tracer_life;
			...
			const auto beam = g_pViewRenderBeams->CreateBeamPoints( beamInfo );
