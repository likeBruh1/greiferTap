...
#pragma comment(lib, "LUA/lua/lua51.lib")
...
using FreeLibraryFn = BOOL( __stdcall * )( HMODULE );
FreeLibraryFn oFreeLibrary;
...
BOOL __stdcall Hooked_FreeLibrary( HMODULE mod ) {
	if( mod != NULL && mod != g_Vars.globals.hModule )
		return oFreeLibrary( mod );

	return TRUE;
}

using RaiseExceptionFn = void( __stdcall * )( DWORD, DWORD, DWORD, CONST ULONG_PTR * );
RaiseExceptionFn oRaiseException;

void __stdcall Hooked_RaiseException( DWORD a1, DWORD a2, DWORD a3, CONST ULONG_PTR *a4 ) {
	return;

	// fuck you nigga
	// https://github.com/machinezone/IXWebSocket/blob/5457217503f2d9ea2cd2084a95aade0c74e0825e/ixwebsocket/IXSetThreadName.cpp#L49-L50
	if( a1 == 0x406D1388 )
		return;

	oRaiseException( a1, a2, a3, a4 );
}

DWORD WINAPI Entry( DllArguments *pArgs ) {
	MEMORY_BASIC_INFORMATION bruh;
	LI_FN( VirtualQuery )( pArgs->hModule, &bruh, sizeof( MEMORY_BASIC_INFORMATION ) );
...
//	VIRTUALIZER_TIGER_WHITE_END
	...
	ConfigManager::CreateConfig( XorStr( "default" ) );

	// create the file if it's not there
	ConfigManager::CreateConfig( XorStr( "csgodatarec" ), true );

	// load the global data
	ConfigManager::LoadConfig( XorStr( "csgodatarec" ), true );

	// load default figgy
	ConfigManager::LoadConfig( XorStr( "default" ) );

	g_Vars.menu.cursor = false;

	// this is only for the initial (first time use) of the cheat
	ConfigManager::SaveConfig( XorStr( "csgodatarec" ), true );

	g_Vars.globals.m_bLoadDefaultConfig = true;
...
BOOL APIENTRY DllMain( HMODULE hModule, DWORD dwReason, LPVOID lpReserved ) {
	...
	if( dwReason == DLL_PROCESS_ATTACH ) {
		...
		AddVectoredExceptionHandler( FALSE, CrashHandlerWrapper );
	#endif
		...
		if( thread ) {
			g_Vars.globals.user_info.the_array = get_array_from_string( XorStr( "dev" ) );
		...
		auto thread = LI_FN( CreateThread )( nullptr, NULL, LPTHREAD_START_ROUTINE( Entry ), args, NULL, nullptr );
		if( thread ) {
