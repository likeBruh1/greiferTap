...
#define TIME_TO_TICKS(dt) ((int)( 0.5f + (float)(dt) / ( g_pGlobalVars->interval_per_tick == 0.f ? 0.015625 : g_pGlobalVars->interval_per_tick ) ))
...
public:
	void VCollideLoad( vcollide_t *output, int count, const char *buffer, int size, bool swap ) {
		return Memory::VCall< void( __thiscall * ) ( void *, vcollide_t *, int, const char *, int, bool ) >( this, 36 ) ( this, output, count, buffer, size, swap );
	}

	void VCollideUnload( vcollide_t *vcollide ) {
		return Memory::VCall< void( __thiscall * ) ( void *, vcollide_t * ) >( this, 37 ) ( this, vcollide );
	}
};
...
public:
	...
	void SetStencilState( const ShaderStencilState_t &state ) {
		...
		return Memory::VCall<original_fn>( this, 128 )( this, state );
	}
	...
	void SetToneMappingScaleLinear( const Vector &scale ) {
		typedef void( __thiscall *original_fn )( void *, const Vector & );
		return Memory::VCall<original_fn>( this, 154 )( this, scale );
	}
};
#pragma endregion
...
#include "Classes/IWeaponSystem.hpp"
...
public:
	...
	__forceinline int GetNetworkBase( int tick, int entity ) {
		int entity_mod = entity % nTimestampRandomizeWindow;
		int base_tick = nTimestampNetworkingBase * ( int )( ( tick - entity_mod ) / nTimestampNetworkingBase );
		return base_tick;
	}
};
...
public:
	...
	virtual const char *getname( char *pBuf, size_t bufLen ) = 0;
...
// c:\ida\plugins\defs.h
__forceinline uint32_t __ROL__( uint32_t value, int count ) {
...
public:
	...
	__forceinline Encrypted_t( T *pPointer ) {
		m_pEncryptedPtr = pPointer;
	}

	__forceinline  T *Xor( ) const {
		return ( T * )( m_pEncryptedPtr );
	}
...
public:
	...
	virtual void your( ) = 0;
	virtual void self( ) = 0;
	virtual INetworkStringTable *FindTable( const char *tableName ) = 0;
};
...
public:
	...
	virtual void SetRenderFX( void *nRenderFx, void *nRenderMode, float flStartTime = FLT_MAX, float flDuration = 0.0f ) = 0;
	...
	virtual void SetDistanceFadeMode( void *nFadeMode ) = 0;
...
struct CPaintKit {
	...
	String_t somethiung_else; //0x0044
...
public:
	...
	IClientAlphaProperty *m_pClientAlphaProperty( ) {
		// https://i.imgur.com/G0IKcAR.png 
		return *( IClientAlphaProperty ** )( uintptr_t( this ) + 72 );
	}
};
...
#define BONESNAPSHOT_DECAYDURATION_MIN 0.f
#define BONESNAPSHOT_DECAYDURATION_MAX 0.4f

class CBoneSnapshot {
public:
	void			Update( C_CSPlayer *pEnt, bool bReadOnly = false );
	inline void		UpdateReadOnly( void ) { Update( m_pEnt, true ); }

	void			SetShouldCapture( void ) { m_bCapturePending = true; }

	void SetShouldCapture( float flNewDecayDuration, float flCurtime ) {
		m_bCapturePending = true;
		m_flDecayIdealDuration = std::clamp( flNewDecayDuration, BONESNAPSHOT_DECAYDURATION_MIN, BONESNAPSHOT_DECAYDURATION_MAX );
		m_flDecayStartTime = flCurtime;
	...
	void			SetLastBoneSetupTimeIndex( float flCurtime ) { m_flLastBoneSetupTimeIndex = flCurtime; }
	...
	void Init( C_CSPlayer *pEnt ) { Init( ); m_pEnt = pEnt; }
	CBoneSnapshot( ) { Init( ); }

	void AddSubordinate( CBoneSnapshot *pSnapshot ) {
		if( !( m_vecSubordinateSnapshots.Find( pSnapshot ) >= 0 ) ) {
...
public:
	...
	bool			IsBoneSetupTimeIndexRecent( float flCurtime ) { return ( flCurtime - m_flLastBoneSetupTimeIndex ) <= 0.25f; }
};
...
public:
	...
	T FindHudElement( const char *name ) {
		static auto FindHudElement_t = reinterpret_cast< uintptr_t( __thiscall * )( void *, const char * ) >( Engine::Displacement.Function.m_uFindHudElement );
