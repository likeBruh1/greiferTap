...
struct animstate_pose_param_cache_t {
	...
	float GetValue( C_CSPlayer *player );
};
...
public:
	...
	void UpdateLayer( C_AnimationLayer *layer, int sequence, float rate, float cycle, float weight, int index );
	...
	void IncrementLayerWeight( C_AnimationLayer *Layer );
	float GetLayerIdealWeightFromSeqCycle( C_AnimationLayer * );
	int SelectSequenceFromActivityModifier( int iActivity );

};//Size=0x344

struct CCSGOPlayerAnimStateServer {
	const int *m_pLayerOrderPreset;
	bool m_bFirstRunSinceInit;
	float m_flFlashedAmountEaseOutStart;
	float m_flFlashedAmountEaseOutEnd;
	float m_tStandWalkAim[ 5 ];
	float m_tStandRunAim[ 5 ];
	float m_tCrouchWalkAim[ 5 ];
	int m_cachedModelIndex;
	void *m_pPlayer;
	void *m_pWeapon;
	void *m_pWeaponLast;
	...
	float m_flDuckAdditional;
	float m_flRecrouchWeight;
	float m_vecPositionCurrent[ 3 ];
	float m_vecPositionLast[ 3 ];
	float m_vecVelocity[ 3 ];
	float m_vecVelocityNormalized[ 3 ];
	float m_vecVelocityNormalizedNonZero[ 3 ];
	float m_flVelocityLengthXY;
	float m_flVelocityLengthZ;
	float m_flSpeedAsPortionOfRunTopSpeed;
	float m_flSpeedAsPortionOfWalkTopSpeed;
	...
	float m_flTimeOfLastKnownInjury;
	float m_flLastVelocityTestTime;
	float m_vecVelocityLast[ 3 ];
	float m_vecTargetAcceleration[ 3 ];
	float m_vecAcceleration[ 3 ];
	float m_flAccelerationWeight;
	float m_flAimMatrixTransition;
	float m_flAimMatrixTransitionDelay;
	bool m_bFlashed;
