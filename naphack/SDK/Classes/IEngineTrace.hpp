...
public:
	bool ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask ) {
		if( ( C_BaseEntity * )pServerEntity == m_pTarget )
			return true;
		return false;
	}
...
class CTraceFilterSkipTeammates : public CTraceFilter {
public:
	CTraceFilterSkipTeammates( IHandleEntity *skip ) {
		pSkip = skip;
	}

	bool ShouldHitEntity( IHandleEntity *pEntityHandle, int content );
...
using ShouldHitFunc_t = bool( __cdecl * )( void *, int );


class CTraceFilterSimple_game {
public:
	void *m_vmt;
	const void *m_pass_ent1;
	int             m_collision_group;
	ShouldHitFunc_t m_shouldhit_check_fn;

public:
	__forceinline CTraceFilterSimple_game( ) :
		m_vmt{ *reinterpret_cast< void ** > ( ( Memory::Scan( XorStr( "client.dll" ), XorStr( "55 8B EC 83 E4 F0 83 EC 7C 56 52" ) ) + 0x3D ) ) },
		m_pass_ent1{ },
		m_collision_group{ },
		m_shouldhit_check_fn{ } {}

	__forceinline CTraceFilterSimple_game( const void *pass_ent1, int collision_group = COLLISION_GROUP_NONE, ShouldHitFunc_t shouldhit_check_fn = nullptr ) :
		m_vmt{ *reinterpret_cast< void ** > ( ( Memory::Scan( XorStr( "client.dll" ), XorStr( "55 8B EC 83 E4 F0 83 EC 7C 56 52" ) ) + 0x3D ) ) },
		m_pass_ent1{ pass_ent1 },
		m_collision_group{ collision_group },
		m_shouldhit_check_fn{ shouldhit_check_fn } {}

	__forceinline bool ShouldHitEntity( void *entity, int contents_mask ) {
		// note - dex; game is dumb, this gets the real vmt.
		void *real_vmt = *( void ** )m_vmt;

		return Memory::VCall< bool( __thiscall * )( void *, C_CSPlayer *, int ) >( real_vmt, 0 )( real_vmt, ( C_CSPlayer * )entity, contents_mask );
	}

	// note - dex; don't really care about calling the virtuals for these two functions, they only set members in the class for us.
	__forceinline void SetPassEntity( void *pass_ent1 ) {
		m_pass_ent1 = pass_ent1;
