...
namespace SDK::Memory {
	...
	uintptr_t *GetVirtualTablePointer( const char *ModuleName, const char *TableName ) {
		...
		uintptr_t rtti_type_descriptor = Scan( module, IDAPattern.data( ), false );
	...
	std::uintptr_t Scan( const std::string &image_name, const std::string &signature, bool bRequest ) {
		auto image = GetModuleHandleA( image_name.c_str( ) );
		return Scan( ( std::uintptr_t )image, signature, bRequest );
	}

	std::uintptr_t Scan( const std::uintptr_t image, const std::string &signature, bool bRequest ) {
	#ifndef DEV
		auto address = patterns_get_pattern( loader_hash( signature.data( ) ).first );
		if( !address ) {

			// idk about this lmfao
			if( !g_is_cracked ) {
				if( !image ) {
